...
The upper-right cell is the main systems question opened by this separation. A protocol can satisfy its current audit-reproduction condition while selecting against, failing to observe, or failing to resource the costly capacities on which that condition depends in the future. Result 5 turns this qualitative statement into an explicit cross-audit state update and asks when the fast threshold is actually crossed.
...
The falsifiable design question is therefore not merely whether $\lambda_x>1$ today. It is whether the surrounding protocol, staking and client ecosystem jointly close a return loop that keeps independently corrective capacity available across future audits. Result 5 derives the corresponding one-step replacement condition at the fast critical boundary.
