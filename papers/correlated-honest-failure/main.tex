...
\]
Machine-checked witnesses show that \(\lambda_x>1\) does not imply this maintenance condition and that maintenance viability does not imply \(\lambda_x>1\). The two thresholds therefore define a genuine two-timescale phase structure: a system may be correctable now while eroding the capacity required to remain correctable later, or may remain operational while lacking fault-specific independent correction.

These are model extensions, not claims about an observed JAM vulnerability. The attribution result follows from the current Gray Paper verdict rules; the branching, dilution and recurrent-maintenance results inherit declared modeling assumptions. The purpose is to make the additional assumptions required by correlated honest failure and long-run corrective capacity explicit and reviewable by the ELVES and JAM authors.
\end{abstract}
